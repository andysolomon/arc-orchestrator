...
- **`ARC_ORCHESTRATOR_RETRY_POLICY`** selects the per-label retry budget: `off` (default — no behavior change; the fallback traversal is byte-for-byte unchanged), `shadow` (records budget evidence on attempted steps but never blocks), or `active` (enforces a sliding-window attempt cap of 2 attempts per label per 60s and the never-cross-a-price-band-twice-without-a-downgrade rule). See `docs/adr/0008-retry-aware-fallback.md`.
