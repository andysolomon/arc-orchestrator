...
```text
/fable-orchestrator:orchestrate implement the smallest safe change for <TASK>. Prefer GPT-5.5 at high reasoning effort if the work is complex, or GPT-5.6 Sol for taste-sensitive Codex implementation/review; use Composer only if the task is mechanical and well-scoped. Scope changes to <FILES_OR_SUBSYSTEMS>. Run <TESTS>. Do not commit or push. Label the run small-implement-<short-name>.
```
