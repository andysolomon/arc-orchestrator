...
Direct workers never commit, push, merge, deploy, or edit secrets. Explicit `sol-*` and `gpt-6-sol-*` aliases pin Sol; use `workload_class: hard-light` or a Codex model override for automatic selection. `--task-class` is metadata only. If Composer edits files but the runner reports it did not return the required structured result, inspect the worktree and run verification before deciding failure.
