...
```text
/arc-orchestrator:direct-worker codex implement <IMPLEMENTATION_TASK>. Scope changes to <FILES_OR_SUBSYSTEM>. Run <TESTS>. Explicit sol-* and gpt-6-sol-* aliases pin Sol; use workload_class: hard-light or a Codex model override for automatic selection. --task-class is metadata only. Do not commit or push. Label the run direct-codex-implement-<short-name>.
```
