...
| Grok (Grok 4.5) availability fallback runtime | required: `plugins/arc-orchestrator/skills/grok-runtime/SKILL.md` | intentional difference — Cursor has no grok-runtime skill; second-tier availability fallback is reached through direct runner invocation (`--backend composer --route grok-*`) in the direct-worker skill. | intentional difference — Pi has no grok-runtime skill; second-tier availability fallback is documented through explicit `bin/arc-orchestrator run --backend composer --route grok-*` commands in arc-orchestrator. | intentional difference — Copilot has no grok-runtime skill; second-tier availability fallback is documented through explicit `bin/arc-orchestrator run --backend composer --route grok-*` commands in `copilot-instructions.md`. |
