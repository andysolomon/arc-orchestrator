...
| Composer orchestrator mode | required: `plugins/fable-orchestrator/skills/orchestrate/references/routing-policy.md` | intentional difference — Cursor remains Fable-first by default with the documented CC-Fable to Codex-Sol to Cursor-Fable-High parent availability chain; Composer orchestrator mode is a canonical routing-policy opt-in, not a Cursor default. | intentional difference — Pi is Codex 5.6 Sol-first and does not expose Composer as a parent orchestrator; it may invoke composer/implement only as a bounded worker route. | intentional difference — Copilot is Codex 5.6 Terra-first and does not expose Composer as a parent orchestrator; it may invoke composer/implement only as a bounded worker route. |
