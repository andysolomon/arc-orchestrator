...
flowchart LR
    ...
    Claude -->|outage| MiniMax["minimax backend<br/>MiniMax-M3 via Claude CLI against the<br/>Anthropic-compatible MiniMax endpoint"]
...
The chain is opt-in for unattended runs: `--fallback claude,minimax` (or `FABLE_ORCHESTRATOR_FALLBACK`) retries each availability-classified failure exactly once on the next tier. MiniMax is a pay-as-you-go API tier (`FABLE_ORCHESTRATOR_MINIMAX_API_KEY` or `MINIMAX_API_KEY`), so it survives subscription exhaustion of both Codex and Claude. `--worker-model <model>` pins the requested backend's model over env and policy; fallback tiers keep their own defaults.
