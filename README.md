...
When a Kimi/Moonshot key is configured (`FABLE_ORCHESTRATOR_KIMI_API_KEY`, `MOONSHOT_API_KEY`, or `KIMI_API_KEY`), the chain gains a terminal tier after MiniMax (or directly after Grok when MiniMax is not configured): the `kimi` backend reuses the Claude Code CLI against Moonshot's Anthropic-compatible endpoint (`ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` are injected per invocation; inherited `ANTHROPIC_API_KEY` is removed from the worker env; default model `kimi-k3[1m]`). Kimi is always terminal. The `kimi` backend is also directly selectable with `--backend kimi` for all three modes.
...
| `FABLE_ORCHESTRATOR_KIMI_MODEL` | `kimi-k3[1m]` | Kimi backend model |
| `FABLE_ORCHESTRATOR_KIMI_BASE_URL` | `https://api.moonshot.ai/anthropic` | Kimi/Moonshot Anthropic-compatible endpoint used by the `kimi` backend |
| `FABLE_ORCHESTRATOR_KIMI_API_KEY` | unset (falls back to `MOONSHOT_API_KEY`, then `KIMI_API_KEY`) | Pay-as-you-go Kimi/Moonshot API key; enables the `kimi` backend and terminal fallback tier |
