...
- Use `--backend codex --mode implement` for harder work or escalation, `--backend codex --mode analyze` for workspace-write-capable codebase exploration, and `--backend codex --mode review` for independent read-only validation. There are no `codex-*` agents or route aliases.
...
- When Claude/Opus is also unavailable, re-delegate to `grok-explore`, `grok-check`, or `grok-implement` on the composer backend with Cursor Grok 4.7 High; Grok is availability recovery, not taste escalation and not a substitute for `opus-review`.
