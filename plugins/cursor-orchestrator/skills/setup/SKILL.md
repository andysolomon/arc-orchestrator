...
`gpt-5.5` is the Codex implement/review default; `gpt-5.6-terra` and `gpt-5.6-luna` are additional Codex worker choices. `gpt-5.6-sol` is reached via explicit `sol-implement` for bounded taste-sensitive Codex work (or a model override); `task_class` never selects a model. Composer defaults to `composer-2.5`, with `ARC_ORCHESTRATOR_COMPOSER_MODEL=gpt-5.6-sol` as an explicit override escape hatch, not the default. The doctor reports availability; explicit model overrides still determine the selected worker model.
