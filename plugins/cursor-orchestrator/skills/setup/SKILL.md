...
- **Codex CLI**: installed on `PATH` and authenticated (`codex login status` under the hood). When authenticated, reports `gpt-5.5`, `gpt-5.6-luna`, and `gpt-5.6-sol` as available in the `codex.models` block.
...
`gpt-5.5` is the Codex implement/review default and `gpt-5.6-luna` is the Codex analysis default. Explicit `sol-*` and `gpt-5.6-sol-*` aliases pin `gpt-5.6-sol`; automatic selection uses `workload_class: hard-light` or a Codex model override for bounded taste-sensitive Codex work. `task_class` never selects a model. Composer defaults to `composer-2.5`, with `ARC_ORCHESTRATOR_COMPOSER_MODEL=gpt-5.6-sol` as an explicit override escape hatch, not the default. The doctor reports availability; explicit model overrides still determine the selected worker model.
