...
function renderComposerEconomyModeGuidance(
  ...
  const parentGuidance = `On ${hostSurface}, this selects the economy worker routes but does not turn the current chat into an Eco parent. True Eco-parent orchestration requires Cursor: open an active Cursor Composer chat and select the same runner identity there.`;
...
Eco orchestrator mode is an explicit opt-in economy mode for a Cursor-native Eco parent. Cursor carries this required policy because Eco-parent orchestration is Cursor-native. It is inactive by default and does not change the ${CURSOR_PARENT_AVAILABILITY_CHAIN} parent availability chain.
...
${renderCursorEcoOrchestratorModeSection()}
...
3. Route: Composer 2.5 for clear mechanical implementation, GPT-5.5 for hard Codex implement/review, GPT-5.6 Luna for repo exploration, GPT-5.6 Sol for ${OPUS_VS_SOL_DISTINCTION.sol}, and Opus 5 for ${OPUS_VS_SOL_DISTINCTION.opus}. \`ARC_ORCHESTRATOR_COMPOSER_MODEL=gpt-5.6-sol\` is an explicit Composer override, not the default. ${EXPLICIT_OVERRIDE_RULE}
...
export function renderCursorOrchestrateEcoCommand(): string {
  return `---
name: orchestrate-eco
description: Orchestrate the given task in explicit Eco-parent economy mode, routing analyze to opus-explore, implement to composer-implement, and review to opus-check.
---
...
\`copy\`, or \`api-design\`). Composer 2.5 remains the default Cursor implementation
...
/** Pi and Copilot skill/prompt runner examples use the shared arc-orchestrator wrapper. */
const RUNNER_WRAPPER_INVOCATION = "bin/arc-orchestrator";

const RUNNER_OVERRIDE_ONLY_PARAGRAPH =
...
${RUNNER_OVERRIDE_ONLY_PARAGRAPH}`;
}
...
For UI/UX, user-facing copy, API design, or other taste-sensitive implement tasks, use ${SOL_REACHABILITY}. \`--task-class\` is observability metadata only and never selects a model.

Inspect recent runs:

\`\`\`sh
${RUNNER_WRAPPER_INVOCATION} runs --limit 10
\`\`\`
...
6. the best route: codex/analyze (GPT-5.6 Luna), codex/implement (GPT-5.5), codex/review (GPT-5.5), or automatic implement with \`workload_class: hard-light-work\` when Sol is required, or composer/implement (Composer 2.5). \`ARC_ORCHESTRATOR_COMPOSER_MODEL=gpt-5.6-sol\` is an explicit Composer override, not the default. ${EXPLICIT_OVERRIDE_RULE_INLINE};
...
${renderRunnerWrapperSection("Invoke the arc-orchestrator wrapper.")}
