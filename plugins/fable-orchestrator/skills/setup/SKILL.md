...
- When a backend is authenticated, report its models from the `models` block (`gpt-5.6-luna`, `gpt-5.6-terra`, and `gpt-5.6-sol` on Codex; `composer-2.5` on Composer). When unauthenticated, do not claim those models are available.
