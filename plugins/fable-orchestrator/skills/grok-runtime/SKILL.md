...
```sh
fable-orchestrator run --backend composer --mode analyze --route grok-explore --task "<task contract>" --cwd "$PWD" --label "<short safe label>"
fable-orchestrator run --backend composer --mode implement --route grok-implement --task "<task contract>" --cwd "$PWD" --label "<short safe label>"
fable-orchestrator run --backend composer --mode review --route grok-check --task "<task contract>" --cwd "$PWD" --label "<short safe label>"
```
