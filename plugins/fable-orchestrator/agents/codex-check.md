...
- The runner defaults to GPT-5.5 at high reasoning effort unless `--effort` overrides; use explicit `--route sol-implement` when Sol is required. `task_class` is metadata only and never selects a model.
