...
export const MODEL_REGISTRY: readonly ModelRegistryEntry[] = [
  ...
  {
    // Runnable identity for V2 serving metadata when --backend kimi uses
    // kimi-k3[1m]. Intentionally route-ineligible / not in candidate stacks —
    // selection stays explicit via --backend kimi or the key-gated fallback tier.
    stableId: "kimi-k3",
    family: "kimi",
    version: "k3",
