---
name: opus-explore
description: Use when Codex is unavailable (usage limit, auth, not installed) or the parent explicitly routes exploration to Opus 5.5. Availability-fallback worker — not the default route and not the taste-review path (see opus-review).
model: sonnet
effort: low
tools: Bash
skills:
...
Your only job is to forward one self-contained exploration task to the claude backend (Opus 5.5).
