...
function usage(): string {
  ...
  return [
    ...
    "  ARC_ORCHESTRATOR_KIMI_MODEL (direct --backend kimi / fallback after the V4 emergency tail minimax-m3 then composer-2.5; default kimi-k3[1m])",
    ...
    "  ARC_ORCHESTRATOR_TRACE_DIR (default ~/.arc-orchestrator/traces)",
...
function appendTrace(record: TraceRecord): void {
  ...
  try {
    ...
    console.error(
      `arc-orchestrator: failed to write trace record: ${errorSummary(error)}`,
    );
  }
}
...
function appendRoutingTraceV2(record: RoutingTraceV2): void {
  ...
  try {
    ...
    console.error(
      `arc-orchestrator: failed to write routing-trace-v2 record: ${errorSummary(error)}`,
    );
  }
}
...
function runAnnotate(args: string[]): void {
  ...
  if (runSelector === "latest") {
    ...
    console.error(
      `arc-orchestrator: no local run ${runSelector}; recording annotation anyway`,
    );
  }
...
function resolveLaminarConfig(): LaminarConfig | null {
  ...
  if (!apiKey) {
    console.error(
      "arc-orchestrator: ARC_ORCHESTRATOR_LAMINAR=1 requires LMNR_PROJECT_API_KEY; skipping Laminar export",
    );
    return null;
  }
  ...
  return {
    ...
    groupName: process.env.LMNR_PROJECT_NAME?.trim() || "arc-orchestrator",
  };
}
...
async function exportRunToLaminar(record: TraceRecord): Promise<void> {
  ...
  try {
    ...
    if (typeof projectId === "string") {
      console.error(
        `arc-orchestrator: laminar: ${laminarFrontendUrl(config.baseUrl)}/project/${projectId}/evaluations/${evaluationId}`,
      );
    }
  } catch (error) {
    console.error(
      `arc-orchestrator: Laminar export failed: ${errorSummary(error)}`,
    );
  }
}
...
function fail(message: string, exitCode = 2): never {
  console.error(`arc-orchestrator: ${message}`);
  console.error(usage());
