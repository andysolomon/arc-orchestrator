  ...
  try {
    ...
    if (
      ...
      if (classified) {
        ...
        emitStderr(`arc-orchestrator: ${message}`);
      }
    } else {
      emitStderr(`arc-orchestrator: ${message}`);
    }
...
async function executeEcoRun(
  ...
  const emitStderr = options.emitStderr ?? console.error;
  emitStderr(
    `arc-orchestrator: progress: eco availability backup ${backupRoute.route} after ${economyRoute.route}`,
  );
...
export async function executeRun(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (shouldRetryToClaude) {
      ...
      emitStderr(
        `arc-orchestrator: progress: primary backend unavailable; retrying on claude fallback`,
      );
      emitStderr(
        `arc-orchestrator: codex unavailable (${attemptResult.outageReason}); retrying on claude backend`,
      );
      currentBackend = "claude";
    ...
    if (shouldRetryToGrok) {
      ...
      emitStderr(
        `arc-orchestrator: progress: Claude backend unavailable; retrying on Grok fallback`,
      );
      emitStderr(
        `arc-orchestrator: claude unavailable (${attemptResult.outageReason}); retrying on composer backend with ${grokProfile.model}`,
      );
      currentBackend = "composer";
    ...
    if (shouldRetryToMinimax) {
      ...
      emitStderr(
        `arc-orchestrator: progress: Grok fallback unavailable; retrying on MiniMax fallback`,
      );
      emitStderr(
        `arc-orchestrator: composer unavailable (${attemptResult.outageReason}); retrying on minimax backend with ${minimaxProfile.model}`,
      );
    ...
    if (shouldRetryToKimi) {
      ...
      emitStderr(
        minimaxTierEnabled
          ? `arc-orchestrator: progress: MiniMax fallback unavailable; retrying on Kimi fallback`
          : `arc-orchestrator: progress: Grok fallback unavailable; retrying on Kimi fallback`,
      );
      emitStderr(
        minimaxTierEnabled
          ? `arc-orchestrator: minimax unavailable (${attemptResult.outageReason}); retrying on kimi backend with ${kimiProfile.model}`
          : `arc-orchestrator: composer unavailable (${attemptResult.outageReason}); retrying on kimi backend with ${kimiProfile.model}`,
      );
      currentBackend = "kimi";
...
async function rejectCanonicalSelection(
  ...
  (options.emitStderr ?? console.error)(`arc-orchestrator: ${params.detail}`);
