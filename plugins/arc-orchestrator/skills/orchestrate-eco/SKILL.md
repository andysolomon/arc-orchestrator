...
```sh
arc-orchestrator run --orchestrator eco --mode implement --task "<bounded implementation contract>" --cwd "$PWD" --label "composer-implement-<short-name>"
```

```sh
arc-orchestrator run --orchestrator eco --mode review --task "<bounded read-only check contract>" --cwd "$PWD" --label "composer-check-<short-name>"
```
