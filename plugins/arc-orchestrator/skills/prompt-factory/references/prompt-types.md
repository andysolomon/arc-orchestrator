...
| `opus-review.md` | `opus-review` | Repo has UI, APIs, docs, prompts, or long-lived abstractions | Show slash commands for Opus 5.5 high-taste read-only critique. |
...
- Use `/arc-orchestrator:setup` first to check backend readiness.
- Use `/arc-orchestrator:observability` to inspect recent delegated worker runs.
- Use `/arc-orchestrator:orchestrate <bounded task>` for repo scans, reviews, implementations, and test-strategy discovery.
