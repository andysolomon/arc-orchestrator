...
Create repo-specific prompt files under `docs/orchestrator/` as simple lists of copy/paste slash commands for the user's current orchestrator surface. Default to Claude Code TUI when this skill is invoked from Claude Code; switch to Pi or Copilot only when the user asks for that surface.
...
5. **Generate.** Ensure `docs/` and `docs/orchestrator/` exist, then write concise `.md` files that are primarily command lists, not long explanations. Completion: every generated file gives multiple copy/paste commands for the selected surface, with labels and safety boundaries embedded in the commands.
