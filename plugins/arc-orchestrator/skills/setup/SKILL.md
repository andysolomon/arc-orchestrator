...
- When a backend is authenticated, report its models from the `models` block (`gpt-5.5`, `gpt-6-luna`, and `gpt-6-sol` on Codex; `composer-2.5` on Composer). When unauthenticated, do not claim those models are available.
