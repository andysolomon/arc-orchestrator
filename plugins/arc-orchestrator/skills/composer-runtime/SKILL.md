...
Bulk mechanical work stays on Composer 2.5. For flagship Sol, prefer automatic `--mode implement` with an appropriate `--workload-class` (or a non-empty `ARC_ORCHESTRATOR_COMPOSER_MODEL=gpt-6-sol` override for local Composer experiments). `task_class` never selects a model. Sol is no longer the Composer default.
