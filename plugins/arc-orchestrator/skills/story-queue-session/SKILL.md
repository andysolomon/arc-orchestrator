...
4. Delegate bounded work to orchestrator workers, scoped to the worktree (`--cwd <story.worktree>`):
   - `arc-orchestrator:arc-delegate` — default automatic runner-routing-v4 worker; pass `--phase` and the implementation workload class while leaving provider selection to the runner.
...
The parent owns scope, approval, and shipping. Ordinary workers remain prohibited from git/GitHub mutations. When the user authorizes shipping, the parent performs `git` and `gh` operations directly in `story.worktree` after reviewing worker evidence. There are no mechanical worker routes.
...
After the loop, adapt selected runner records into the current Story Queue `RunRecord` contract:

```bash
arc-orchestrator runs --json --limit 20 | bun plugins/orchestrator-core/trace-adapter.ts --story <story-id> --repo owner/name --run <run-id> > /tmp/runs.json
```
...
Pipe `arc-orchestrator runs --json` through the trace adapter before calling `story.complete`:

```bash
arc-orchestrator runs --json --limit 10 | bun plugins/orchestrator-core/trace-adapter.ts --story <story-id> --repo owner/name --run run-abc --run run-def > /tmp/runs.json
```
