...
| Reasoning-effort routing | Missing | Selection operates on rungs `(stableId, effort)` rather than bare models, so the cost/quality frontier inside one model is reachable. Today `--effort` is a Codex-only spawn flag absent from the registry, stacks, and shadow selection (Phase 13) |
...
- The lead of every derived stack shares a `transportBackend` with the incumbent lead recorded at migration, unless displaced by a strictly higher band or by availability. Specifically: `grok-4.5` does not take the `medium-work` lead from `gpt-5.5` on its 8.3-point CursorBench margin, and does not take a lead from `opus-5` where the two tie at 66.7% — reproducing #237's decisions rather than reversing them.
