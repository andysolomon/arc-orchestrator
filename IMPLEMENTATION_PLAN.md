...
- An effort-tier identity rule: a measurement is authoritative only for the tier it was captured at, so a `max` leaderboard column may never characterize a rung dispatched at `high` or below. *Met (13.9), and it immediately named a live defect — see 13.9a.*
