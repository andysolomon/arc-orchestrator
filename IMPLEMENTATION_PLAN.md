...
- `AvailabilityView` including `quotaPools`, where quota is ordering input only; `BudgetDimension`, the reserve/reconcile math, and every `RoutingTraceV2` budget field are unchanged, and `budget-limits/v1` remains the sole admission authority. *Met (13.5): `availability-view.ts` builds the view from timestamped observations; the budget invariant is asserted rather than described.* Two rules the ADR left open. Observations must expire, and **quickly**: `select()` treats `unavailable` as a hard rejection, so the verdict blocks the dispatch that would observe recovery — held too long it is self-reinforcing, and a backend that came back is never retried. The window is 60s, matching `RETRY_BUDGET_DEFAULT_WINDOW_MS`. And not every failure class describes a backend: every terminal class (`policy_denial`, `sandbox_incompatible`, `invalid_configuration`, `deterministic_validation_error`) describes the *request*, so mapping any of them onto health would let one malformed request take a provider out of rotation for every task. Quota decays toward unobservable rather than toward zero, since zero is the one value that rejects. Nothing produces observations yet; 13.10 wires the producer.
...
- A `workload_class` to `capabilityFloor` mapping accepted alongside the existing classes during migration, with `workload_class` demoted to observability metadata. *Met (13.8): `capability-floor.ts`, `capability-floor/v1`.* It is not the table both ADRs assume, and the reasons are facts about the class vocabulary rather than preferences. **The floors are derived, not authored**: a class's floor is the band its authored lead already occupies, resolved through the same `candidateStackForRoute` the live router uses, read off the *lowest* ranked rung of that lead because a `stableId` spans several and today's dispatcher may run any of them — a stricter floor than the thing being migrated would start refusing work the system does today. **The ladder is not monotone and was never meant to be**: `medium-light-work`/`medium-work` and `hard-light-work`/`hard-work` hold identical candidate sets with the first two swapped, so `-light-` marks usage headroom rather than difficulty, and where the leads differ in band the *lighter* class legitimately sits higher — the trade `test/workload-ladder.test.ts` already excuses; meanwhile `medium-hard-work` and `hard-work` both lead with `fable-5`, so no snapshot can ever separate them. Seven names, five leads, one sanctioned inversion. **Degradation latitude is already authored as `automaticFallback`** — a fallback stack runs its tail to the weakest member, so `minimumFloor: 0` is the faithful translation and a pinned stack does not degrade; migration grants no latitude the stacks did not have, and widening it stays ADR 0011's `TaskBudgetPolicy` decision. **Two classes state a ceiling, not a floor**: `default` and `light-work` pin one cheap candidate with no fallback, and carrying only a floor inverts them, because floor 0 admits everything and `select()` orders by band descending — so the cost-pinned classes would lead with the *most* capable rung. That is a correctness requirement with a test showing the uncapped selection picking the dearest rung. Two consequences: deriving the floor is what keeps ADR 0010's stated rollback real, since `select()` rejects an unranked rung against any floor above 0 and a guessed floor would turn "delete the snapshot" into a refusal of every medium-and-up dispatch (with no snapshot every floor is 0 and the cost pins lose their ceiling too — that is what rollback costs); and a derived floor can never be unreachable, because it is always a band some rung occupies, where an authored table could have assigned `hard-work` a floor of 4 that needs a score above 0.8 no suite approaches, refusing everything while looking like policy. Dual acceptance: an explicit floor wins and the derived one is returned beside it, because the migration question is whether they agree; an explicit floor does not inherit the class's ceiling. The demotion is enforced, not asserted — `capability-selection.ts` contains no reference to `workload_class` and a test fails if one appears.
